...
class TournamentController:
    ...
    def show(self, tournament_id: str) -> None:
        ...
        if tournament.current_round > 0:
            ...
            if tournament.current_round < tournament.number_of_rounds:
                current_round_id = sorted_rounds[
        ...
        menu_options = {
            key: option["name"] for key, option in tournament_menu.items()
        }
        while True:
    ...
    def start_round(self, tournament_id: str) -> None:
        ...
        tournament = TournamentModel.load_by_id(tournament_id)
        t_players = PlayerModel.get_tournament_players(
            tournament_id=tournament.get_id()
        )
        if not t_players:
    ...
    def archive_tournament(self, tournament_id: str) -> str:
        ...
        t_players_dict = [
            {"player": p["player"].__dict__, "player_score": p["player_score"]}
            for p in t_players
        ]
        t_rounds = RoundModel.get_tournament_rounds(
            tournament_id=tournament.get_id()
        ...
        for r in t_rounds_dict:
            games = GameModel.get_rounds_games(
                round_id=f"{r['round_id']}.json"
            )
            if games:
                r["games"] = [g.to_dict() for g in games]
        t_dict = tournament.__dict__
        t_dict["players"] = sorted(
            t_players_dict, key=lambda p: p["player_score"], reverse=True
        )
        t_dict["rounds"] = t_rounds_dict
        try:
            with open(
                f"data/archives/{t_dict['starts']}_{t_dict['name']}.json", "w"
            ) as json_file:
                json.dump(t_dict, json_file)
            PlayerModel.remove_tournament_players(
                tournament_id=tournament.get_id()
            )
            tournament.remove()
            for r in t_rounds:
            ...
            alert_message(message=str(e), type="Error")
        alert_message(
            message="Tournoi Archivé, vous pourez le consulter depuis le menu principal.",
            type="Info",
        )
        return "Exit"
    ...
    def load_tournament_menu(
            ...
            menu = {
                "1": {
                    ...
                    "controller": lambda: PlayerController().show_tournament_players(
                        tournament_id=tournament_id, option="player_list"
                    ),
                },
                "2": {
        ...
        if current_round >= 1:
            menu = {
                ...
                "2": {
                    ...
                    "controller": lambda: RoundController().show_rounds(
                        tournament_id=tournament_id
                    ),
                },
            }
        ...
        menu["q"] = {"name": "Quitter", "controller": lambda: None}
        return menu
