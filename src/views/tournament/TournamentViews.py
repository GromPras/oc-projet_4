...
#{p["player"]["national_chess_id"]} - {p["player_score"]} points"""
            )
        print("_" * 80)
        print("Matchs : ")
        for index, r in enumerate(
            sorted(archived_tournament["rounds"], key=lambda r: r["name"]), 1
        ):
            print(
                f"""{r["name"]} Début: {r["started_on"].split('.')[0]} - Fin: {r["ended_on"].split('.')[0]}"""
            )
            print("-" * 40)
            for g in r["games"]:
                print(
                    f"""{g["player_1"]} (score: {g["player_1_score"]}) \
